...
# change prior for delta: beta or gamma
...
sampinfo::getStanCode(deltaM_value = 0)
```

Next let's add a flexible decay parameter.
```{r, comment = ">", highlight=FALSE}
sampinfo::getStanCode(deltaM_value = 9)
```

Some tips on how to programming multiple models in a single stan file.
```{r, comment = ">", highlight=FALSE}
sampinfo::getStanCode(deltaM_value = 1, hier_value = 0)
```
...
print(head(rhat[rhat>1.05&!is.na(rhat)]))
```
...
- Stan manual: [Link1](https://mc-stan.org/docs/2_23/functions-reference/index.html)
[Link2](https://mc-stan.org/docs/2_23/reference-manual/index.html)
